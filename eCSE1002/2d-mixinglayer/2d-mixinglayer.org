...
#+BEGIN_SRC js :tangle input.json
  {
    ...
    "rho": {
      ...
      "properties": {
        "filename": "/home/paul/DATA/incompact3d-lmn/golanski_mixinglayer2d/DATA/test_lamballais/e8/rho",
        "direction": [0]
      }
    }
  }
...
#+BEGIN_SRC python
  ...
  cs = plt.contour(X, Y, postprocess.fields["rho"].data[t][:,:,0],
                   contours,
                   colors="r")
  filename = "e8-t" + str(t) + ".eps"
  plt.savefig(filename)
  filename = "e8-t" + str(t) + ".png"
  plt.savefig(filename)
  plt.close
...
\begin{figure}[h]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[height=0.45\textheight]{e7-t182}
    \caption{e7}
  \end{subfigure}
  \begin{subfigure}[b]{0.3\textwidth}
    \includegraphics[height=0.45\textheight]{e8-t182}
    \caption{e8}
  \end{subfigure}
  \caption{Comparison of results using conservative formulation with Golanski2005}
\end{figure}
#+END_EXPORT
