...
!############################################################################
!
subroutine STATISTIC(rho1, ux1, uy1, uz1, phi1, ta1, rhomean, umean, vmean, wmean, &
     ufmean, vfmean, wfmean, phimean, uumean, vvmean, wwmean, &
...
real(mytype),dimension(xsize(1),xsize(2),xsize(3)), intent(in) :: rho1,ux1,uy1,uz1,phi1
real(mytype),dimension(xszS(1),xszS(2),xszS(3)) :: rhomean,umean,vmean,wmean,uumean,vvmean,wwmean,uvmean,uwmean,vwmean,tmean
real(mytype),dimension(xszS(1),xszS(2),xszS(3)) :: ufmean,vfmean,wfmean
...
if (mod(itime,isave)==0) then
   ...
   if (ilmn.ne.0) then
      call decomp_2d_write_one(1, rhomean, "rhomean.dat", 1)
      
      call decomp_2d_write_one(1, ufmean, "ufmean.dat", 1)
      call decomp_2d_write_one(1, vfmean, "vfmean.dat", 1)
      call decomp_2d_write_one(1, wfmean, "wfmean.dat", 1)
      
      if (nrank==0) print *,'write stat arrays LMN done!'
   endif
...
!! Includes for mixing layer case
!
!  rstress.inc - Reynolds stresses
!  rdiss.inc - Reynolds dissipation rate
!  thickness.inc - Mixing layer thickness
#include "rstress.inc"
#include "rdiss.inc"
#include "thickness.inc"
