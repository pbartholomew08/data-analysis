...
#+BEGIN_SRC f90
  ...
  if (ilmn.ne.0) then
     ta1(:,:,:) = rhomean(:,:,:) * (0.5_mytype * (u1 - u2) - ufmean(:,:,:) / REAL(itime, mytype)) &
          ,* (0.5_mytype * (u1 - u2) + ufmean(:,:,:) / REAL(itime, mytype))
  else
     ta1(:,:,:) = (0.5_mytype * (u1 - u2) - ufmean(:,:,:) / REAL(itime, mytype)) &
          ,* (0.5_mytype * (u1 - u2) + ufmean(:,:,:) / REAL(itime, mytype))
  endif

  print *, MAXVAL(ta1), MINVAL(ta1)

  !! Now average in X
  DO i = 2, xsize(1)
     ta1(1,:,:) = ta1(1,:,:) + ta1(i,:,:)
  ENDDO
  ta1(1,:,:) = ta1(1,:,:) / REAL(nx, mytype)
  DO i = 2, xsize(1)
     ta1(i,:,:) = ta1(1,:,:)
  ENDDO
...
#+BEGIN_SRC f90
  ...
  ENDDO
  deltam = deltam / (((dens1 + dens2) / 2._mytype) * (u1 - u2)**2)
#+END_SRC
...
#+BEGIN_SRC f90
  !! Get Favre-avg u to Y
  CALL transpose_x_to_y(ufmean, tb2)
